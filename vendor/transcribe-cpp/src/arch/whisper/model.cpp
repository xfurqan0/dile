...
transcribe_status whisper_run(transcribe_session *          session,
    ...
    // Real, unpadded mel frames. Short-form pads the PCM to 30 s *before* the
    // mel, so total_mel_frames is 3000 no matter how short the take was; the
    // audio_ctx floor below has to know how much of that is speech and how
    // much is the zeros we put there.
    const int mel_hop_samples = (n_samples_per_chunk > 0 && n_mel_frames_per_chunk > 0) ?
                                    n_samples_per_chunk / n_mel_frames_per_chunk :
                                    160;
    const int real_mel_frames =
        is_short_form ?
            std::min(n_mel_frames_per_chunk,
                     (n_samples + std::max(1, mel_hop_samples) - 1) / std::max(1, mel_hop_samples)) :
            total_mel_frames;
    ...
    while (seek < total_mel_frames) {
        ...
        // How wide the encoder actually runs. Default is the full 30 s window;
        // wp->audio_ctx (whisper.cpp's -ac) shortens it to 2*audio_ctx mel
        // frames, which is what stops a three-second take from paying for
        // twenty-seven seconds of zeros. Two floors are not negotiable:
        //   - never below the real (unpadded) audio in THIS window, so the
        //     knob can only ever drop padding, never speech;
        //   - even, because the conv2 stem has stride 2 and T_enc =
        //     n_mel_frames / 2.
        // seek_num_frames stays the real window length: timestamps and the
        // seek advance below are about the audio, not about the encoder.
        int enc_frames = n_mel_frames_per_chunk;
        if (wp->audio_ctx > 0 && wp->audio_ctx < cm->hparams.enc_max_source_positions) {
            const int frames_real_here = std::max(0, std::min(real_mel_frames - seek, n_mel_frames_per_chunk));
            enc_frames                 = std::max(wp->audio_ctx * 2, frames_real_here);
            enc_frames                 = std::min(enc_frames + (enc_frames % 2), n_mel_frames_per_chunk);
        }

        std::vector<float> chunk_mel(static_cast<size_t>(n_mels) * static_cast<size_t>(enc_frames), 0.0f);
        const int          frames_avail = std::min(seek_num_frames, enc_frames);
        ...
        if (const transcribe_status st = run_whisper_encoder_on_window(cc, cm, chunk_mel.data(), n_mels, enc_frames,
