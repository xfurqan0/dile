...
struct transcribe_whisper_run_ext {
     ...
     *
     * Whisper's encoder always runs a fixed 30-second window: short audio is
     * zero-padded to 480000 samples / 3000 mel frames / 1500 encoder
     * positions, and every one of those positions is attended over. A
     * three-second utterance therefore pays the same encoder cost as a
     * thirty-second one, and on this model that encoder pass is 80-93% of
     * the wall clock.
     *
     * Setting audio_ctx to N runs the encoder over the first N positions
     * only (2N mel frames), taking a prefix view of the positional
     * embedding. The library never truncates below the real, unpadded
     * audio in the window, so the frames dropped are padding.
     *
     *   0 (default)  full window, unchanged behavior.
     *   >= 1500      same as 0.
     *   1..1499      truncate, floored at the real audio in the window.
     *
     * NOT a free dial. Too small a window pushes the decoder into a
     * repetition loop, which trips compression_ratio_thold, which starts
     * the temperature fallback ladder, which costs far more than the
     * encoder saved. Measured safe floors: ~512 for 3 s, ~768 for 11 s;
     * a fixed 512 on a 30 s take ran 8x slower than the default AND
     * hallucinated. Scale it with the audio: roughly 67 positions per
     * second of speech, floored at 512.
     */
    int32_t audio_ctx;
};
